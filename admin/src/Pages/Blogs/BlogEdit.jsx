...
const BlogEdit = () => {
  ...
  useEffect(() => {
    getData(`/api/blog/${id}`).then((res) => {
      if (res?.error !== true) {
        ...
        setPreviews(res?.blog?.image || []);
      } else {
        context.openAlertBox("error", res?.message, "getBlogByID-error");
        navigate("/blogs");
      }
    });
  }, [id]);
  ...
  const onChangeDesc = (e) => {
    ...
    formFeilds.desc = e.target.value;
  };
  ...
  const removeImg = (image, index) => {
    ...
    var imageArr = [];
    imageArr = previews;
    deleteData(`/api/blog/deleteImage?imageUrl=${image}`).then((res) => {
      if (res?.error !== true) {
        ...
        setIsRemoveLoading(false);
        context.openAlertBox("error", res?.message, "deleteBlogImage-error");
      }
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (formFeilds.name === "") {
      context.openAlertBox("error", "Enter Blog Name", "MissingBlogName-error");
      setIsLoading(false);
      return;
    }

    editData(`/api/blog/updateBlog/${id}`, formFeilds, {
      withCredentials: true,
    }).then((res) => {
      console.log(res);
      if (res?.error !== true) {
        context.openAlertBox("success", res?.message, "updateBlog-success");
        setFormFeilds({
          name: "",
          images: [],
        });
        ...
        context.openAlertBox("error", res?.message, "updateBlog-error");
        setIsLoading(false);
      }
    });
  };
        ...
        >
          <div className="shadow-md rounded-md p-3 bg-white mt-5">
            <div className="cartHead p-2 pb-4 mb-3 border-b border-[#ff5252]">
              <h2 className="font-bold text-[18px]">EDIT BLOG</h2>
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-center gap-3">
                <div className="name w-full">
                  <div className="name mb-2 rounded-lg overflow-hidden">
                    <Editor
                      value={formFeilds.desc}
                      disabled={isUploading}
